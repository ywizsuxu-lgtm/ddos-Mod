...
def _probe_with_curl_cffi(url, method):
    ...
    try:
        ...
        return {
            "ok": True,
            "backend": "curl_cffi",
            "impersonate": HTTP_PROBE_DEFAULT_IMPERSONATE,
            "url": url,
            "method": method,
            ...
            "ca_bundle": certifi.where() if certifi is not None else "system default",
        }
...
def http_probe(url, method="GET"):
    """Run one bounded HTTP diagnostic request using curl_cffi when available."""
...
def probe():
    ...
    if method not in {"GET", "HEAD"}:
        stdout.write(Fore.RED + " [!] " + Fore.WHITE + "Only GET and HEAD are supported by the diagnostic probe.\n")
        return
        ...
        try:
            ...
            if target['scheme'] == 'https':
               ...
               # ctx = _create_compat_tls_context()
                ...
                s = _create_compat_tls_context().wrap_socket(s, server_hostname=target['host'])
            else:
                s = socks.socksocket()
                s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
...
def help():
    ...
    stdout.write("             "+Fore.LIGHTCYAN_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"probe    "+Fore.LIGHTCYAN_EX+"|"+Fore.LIGHTWHITE_EX+" Run one bounded HTTP diagnostic          "+Fore.LIGHTCYAN_EX+"║\n")
...
def func():
    ...
    stdout.write(Fore.MAGENTA+" • "+Fore.WHITE+"probe      "+Fore.RED+": "+Fore.WHITE+"Single HTTP/HTTPS diagnostic request\n")
